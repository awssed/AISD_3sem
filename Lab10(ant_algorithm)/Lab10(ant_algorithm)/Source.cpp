...
#define PHEROMONE_INCREMENT 1.0
...
)
{
	...
	while (i < nIters)
	{
		...
		else
		{
			...
			for (int q = 0; q < pheramons.size(); q++)
		{
			for (int w = 0; w < pheramons.size(); w++)
			{
				pheramons[q][w] = pheramons[q][w] * 0.9;
			}
		}
		cout << "Итерация: " << i + 1 << endl;
        cout << "Лучший путь: " << best.to_string() << endl;
        cout << "Текущий путь: " << way.to_string() << endl;
        cout << "-------------------" << endl;
	}
	return best;
}
